...
module.exports = (ts) => {
  ...
  ts.test('case: basic perspective projection to plane', (t) => {
    ...
    t.deepEqual(
      vec2.projectTo({ x: 3, y: 0 }, plane, camera),
      { x: 0, y: 0 },
    ...
    t.deepEqual(
      vec2.projectTo({ x: 2, y: 0 }, planeb, camerab),
      { x: 1, y: 0 },
    ...
    t.deepEqual(
      vec2.projectTo({ x: 6, y: 0 }, planec, camerac),
      { x: 1, y: 0 },
